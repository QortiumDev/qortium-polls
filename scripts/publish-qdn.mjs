...
const ENV_PREFIX = 'QORTIUM_POLLS';
const DEFAULT_NODE_API_URL = 'http://127.0.0.1:24891';
const DEFAULT_NAME = 'Polls';
const DEFAULT_IDENTIFIER = 'Polls';
const DEFAULT_TITLE = 'Polls';
const DEFAULT_DESCRIPTION = 'Create, browse, and vote on Qortium on-chain polls — and a living reference for the poll/vote system.';
...
async function ensureNameRegistered(name, account) {
  ...
  const rawRegisterBytes58 = buildRegisterNameRawBytes58({
    ...
    data: JSON.stringify({
      app: DEFAULT_TITLE,
      purpose: 'QDN app preview for Polls',
    }),
  });
